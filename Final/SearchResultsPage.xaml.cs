...
namespace ContosoElectronics
{
    ...
    public sealed partial class SearchResultsPage : Page
    {
        ...
        private async void navigationHelper_LoadState(object sender, LoadStateEventArgs e)
        {
            ...
            var categories = await CategoryDataSource.GetListAsync();
            foreach (var category in categories)
            {
                var products = (_searchResults.Where(item => item.Category.Equals(category.Id)));
                if (products.Count() > 0) filterList.Add(new Filter(category.Name, products.Count(), false));
            }
        ...
        async void Filter_Checked(object sender, RoutedEventArgs e)
        {
            ...
            if (selectedFilter != null)
            {
                ...
                _searchResults = await ProductDataSource.SearchProductsAsync(_queryText);
                ProductCategory category = await CategoryDataSource.GetCategoryDetailsAsync(selectedFilter.Name);
                _searchResults = (selectedFilter.Name.Equals("All")) ? _searchResults :
                    new List<Product>(_searchResults.Where(item => item.Category.Equals(category.Id)));
                this.DefaultViewModel["Results"] = _searchResults;
        ...
        {
            this.Frame.Navigate(typeof(ProductDetailsPage), (e.ClickedItem as Product).Id);
        }
    }
}
